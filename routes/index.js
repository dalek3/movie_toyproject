		 ...
		 if (info.username === results[0].username) {
			 ...
			 res.redirect('/'); 
	 	 	...
	 	 	if (info.password === info.password2) {
				 //전부 맞을때 sql 입력
				connection.query('INSERT INTO  movieinfo SET username = ?, password = ?',
				 [info.username , info.password], function (err) {
...
exports.login = function(req, res){
	connection.query('SELECT * FROM movieinfo WHERE username = ?', [req.body.username], function (err, results, fields) {
